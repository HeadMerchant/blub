...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
  template <typename T> explicit Approx(const T& value, typename detail::types::enable_if<std::is_constructible<double, T>::value>::type* = static_cast<T*>(nullptr)) {
  ...
  template <typename Type, typename... Rest> struct iter<std::tuple<Type, Rest...>> {                                                                                    \
    iter(const char* file, unsigned line, int index) {                                                                                                                   \
      doctest::detail::regTest(                                                                                                                                          \
        doctest::detail::TestCase(func<Type>, file, line, doctest_detail_test_suite_ns::getCurrentTestSuite(), doctest::toString<Type>(), int(line) * 1000 + index) *    \
        dec                                                                                                                                                              \
      );                                                                                                                                                                 \
...
Subcase::Subcase(const String& name, const char* file, int line) : m_signature({name, file, line}) {
  if (!g_cs->reachedLeaf) {
    ...
    if (g_cs->subcaseStack[g_cs->currentSubcaseDepth] == m_signature) {
      ...
      g_cs->fullyTraversedSubcases.find(hash(hash(g_cs->subcaseStack, g_cs->currentSubcaseDepth), hash(m_signature))) == g_cs->fullyTraversedSubcases.end()
    ) {
      if (checkFilters()) {
        return;
      }
...
struct ConsoleReporter : public IReporter {
  ...
  void test_run_end(const TestRunStats& p) override {
    ...
    auto passwidth = int(
      std::ceil(log10(static_cast<double>(std::max(p.numTestCasesPassingFilters - p.numTestCasesFailed, static_cast<unsigned>(p.numAsserts - p.numAssertsFailed))) + 1))
    );
    ...
    s << Color::Cyan << "[doctest] " << Color::None << "assertions: " << std::setw(totwidth) << p.numAsserts << " | "
      << ((p.numAsserts == 0 || anythingFailed) ? Color::None : Color::Green) << std::setw(passwidth) << (p.numAsserts - p.numAssertsFailed) << " passed" << Color::None
...
#define DOCTEST_PARSE_AS_BOOL_OR_FLAG(name, sname, var, default)                                                                                                         \
  if (                                                                                                                                                                   \
    parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_bool, intRes) ||                                                                           \
    parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", option_bool, intRes)                                                                             \
  )                                                                                                                                                                      \
    p->var = static_cast<bool>(intRes);                                                                                                                                  \
  else if (parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name) || parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname)) p->var = true;                       \
  else if (withDefaults) p->var = default

#define DOCTEST_PARSE_INT_OPTION(name, sname, var, default)                                                                                                              \
  if (                                                                                                                                                                   \
    parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_int, intRes) ||                                                                            \
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                                                                                              \
  ...
  if (
    parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "help") || parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "h") ||
...
void registerReporterImpl(const char* name, int priority, reporterCreatorFunc c, bool isReporter) {
  if (isReporter) getReporters().insert(reporterMap::value_type(reporterMap::key_type(priority, name), c));
  else getListeners().insert(reporterMap::value_type(reporterMap::key_type(priority, name), c));
}
} // namespace detail
